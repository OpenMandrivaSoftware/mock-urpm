...
def init(rootObj, conf):
    if mock_urpm.util.selinuxEnabled():
...
class SELinux(object):
    ...
    def __init__(self, rootObj, conf):
        ...
        if self._selinuxYumIsSetoptSupported():
            rootObj.addHook("preurpmi", self._selinuxPreYumHook)
            rootObj.addHook("posturpmi", self._selinuxPostYumHook)
        else:
    ...
    def _selinuxPreYumHook(self):
        self._originalUtilDo = mock_urpm.util.do
        mock_urpm.util.do = self._selinuxDoYum

    decorate(traceLog())
    def _selinuxPostYumHook(self):
        mock_urpm.util.do = self._originalUtilDo
