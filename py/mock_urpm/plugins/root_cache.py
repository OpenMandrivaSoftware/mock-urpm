...
# our imports
from mock_urpm.trace_decorator import decorate, traceLog, getLog
...
class RootCache(object):
    ...
    def __init__(self, rootObj, conf):
        ...
        self.exclude_dirs = ["./proc", "./sys", "./dev", "./tmp/ccache" ]
        self.exclude_tar_cmds = [ "--exclude=" + d for d in self.exclude_dirs]
    ...
    def _rootCachePreInitHook(self):
        ...
        mock_urpm.util.mkdirIfAbsent(self.rootSharedCachePath)
        ...
        if os.path.exists(self.rootCacheFile) and self.rootObj.chrootWasCleaned:
            ...
            for dir in self.exclude_dirs:
                mock_urpm.util.mkdirIfAbsent(self.rootObj.makeChrootPath(dir))
    ...
    def _rootCachePostInitHook(self):
        try:
            ...
            if self.rootObj.chrootWasCleaned:
                mock_urpm.util.do(["sync"], shell=False)
                ...
                l.write(self.rootObj.urpmi_init_install_output)
                l.close()
        finally:
            self._rootCacheUnlock()
