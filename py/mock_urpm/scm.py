...
class scmWorker(object):
    ...
    def __init__(self, log, opts):
        ...
        if self.branch:
            self.chdir = self.chdir.replace("SCM_BRN", self.branch)
        if self.pkg:
            self.chdir = self.chdir.replace("SCM_PKG", self.pkg)
    ...
    def get_sources(self):
        ...
        mock_urpm.util.do(shlex.split(self.get), shell=False, cwd=self.wrk_dir)
        if self.postget:
            mock_urpm.util.do(shlex.split(self.postget), shell=False, cwd=self.src_dir)

        if(self.chdir != ''):
            self.src_dir += '/' + self.chdir
            if not os.path.exists(self.src_dir):
                self.log.error("Can't change work directory to %s. Check 'chdir' config option." % self.src_dir)
                sys.exit(5)
            self.wrk_dir = os.path.dirname(self.src_dir)
    ...
    def prepare_sources(self):
        ...
        self.log.debug("Sources: %s" % str(self.sources))
        ...
        if self.write_tar:
            ...
            mock_urpm.util.do(shlex.split(cmd), shell=False, cwd=self.wrk_dir)
    ...
    def clean(self):
        ...
        mock_urpm.util.rmtree(self.wrk_dir)
